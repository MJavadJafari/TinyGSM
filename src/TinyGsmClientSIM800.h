...
class TinyGsmSim800 : public TinyGsmModem<TinyGsmSim800>,
 ...
 public: 
  // Read the last received SMS
  bool readLastSMS(String& number, String& message) {
    sendAT(GF("+CMGF=1")); // Set SMS text mode
    if (waitResponse() != 1) return false;

    sendAT(GF("+CSCS=\"GSM\"")); // Set GSM character set
    if (waitResponse() != 1) return false;

    sendAT(GF("+CMGL=\"REC UNREAD\",1")); // Read unread SMS messages
    String response;
    if (waitResponse(10000L, response) != 1) return false;

    int start_index = response.indexOf(GF("+CMGL:"));
    if (start_index == -1) return false;
    start_index += 7; // skip +CMGL:

    int end_index = response.indexOf(',', start_index);
    if (end_index == -1) return false;

    int index = response.substring(start_index, end_index).toInt();

    // read sms from index

    bool changeStatusToRead = true;
    ...
    // get number
    start_index = response.indexOf(GF("\",\"")) + 3;
    if (start_index == -1) return false;

    end_index = response.indexOf(GF("\""), start_index);
    if (end_index == -1) return false;

    number = response.substring(start_index, end_index);
