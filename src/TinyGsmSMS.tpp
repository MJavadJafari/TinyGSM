...
class TinyGsmSMS {
 ...
 protected:
  ...
  String sendUSSDImpl(const String& code) {
    ...
    thisModem().sendAT(GF("+CSCS=\"IRA\""));
    thisModem().waitResponse();
    // Send the message
    thisModem().sendAT(GF("+CUSD=1,\""), code, GF("\",15"));
    if (thisModem().waitResponse() != 1) { return ""; }
    if (thisModem().waitResponse(20000L, GF("+CUSD:")) != 1) { return ""; }
  ...
  bool sendSMSImpl(const String& number, const String& text) {
    ...
    thisModem().waitResponse();
    thisModem().sendAT(GF("+CSMP=17,167,0,0"));
    thisModem().waitResponse();
    thisModem().sendAT(GF("+CMGS=\""), number, GF("\""));
    thisModem().waitResponse(GF(">"));
  ...
  bool sendSMS_UTF8_end() {
    ...
    thisModem().stream.flush();
    bool res = thisModem().waitResponse(60000L) == 1;
    // set GSM 7 bit default alphabet (3GPP TS 23.038)
    thisModem().sendAT(GF("+CSCS=\"GSM\""));
    thisModem().waitResponse();
    return res;
